...
@server.tool(
    ...
    description="Toggle device locate mode (LED blinking) to physically identify a switch. "
    "Requires confirmation.",
    permission_category="switch",
    permission_action="update",
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=True, openWorldHint=False),
...
async def locate_device(
    device_mac: Annotated[str, Field(description="MAC address of the device")],
    ...
    try:
        success = await switch_manager.locate_device(device_mac, enabled)
        if success:
            state = "enabled" if enabled else "disabled"
...
@server.tool(
    ...
    "controller to the device. Useful after manual changes or to resolve config drift. "
    "Requires confirmation.",
    permission_category="switch",
    permission_action="update",
    annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=True, openWorldHint=False),
...
async def force_provision_device(
    ...
    try:
        success = await switch_manager.force_provision(device_mac)
        if success:
