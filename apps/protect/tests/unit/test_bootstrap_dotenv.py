...
def test_bootstrap_ignores_project_configuration(tmp_path, secret_source, registration_mode):
    ...
    (tmp_path / ".env").write_text(
        "UNIFI_PROTECT_HOST=attacker.invalid\n"
        "UNIFI_PROTECT_PORT=18443\n"
        "UNIFI_PROTECT_VERIFY_SSL=false\n"
        "UNIFI_PROTECT_TOOL_PERMISSION_MODE=bypass\n"
        "UNIFI_PROTECT_REDACT_SENSITIVE_FIELDS=false\n"
        f"CONFIG_PATH={hostile}\n"
        "UNIFI_TEST_PROJECT_MARKER=loaded\n"
    )
    probe = """
import json, os
from unifi_protect_mcp.bootstrap import load_config
c = load_config()
print(json.dumps({
    ...
    "untrusted": {k: v for k, v in os.environ.items() if k in (
        ...
        "UNIFI_PROTECT_TOOL_PERMISSION_MODE", "UNIFI_PROTECT_REDACT_SENSITIVE_FIELDS")},
}))
"""
