...
description: |
  ...
  in validator_registry.py; (3) the validated_data contract — registry.validate()
  returns a (is_valid, error_msg, validated_data) tuple, not a plain dict, and
  validated_data MUST replace the original dict downstream; (4) field prefix
  correctness (e.g., dhcpd_* not dhcp_* — silent failure on live controller);
  (5) layer-2 hardening with additionalProperties: false on all *_UPDATE_SCHEMA
  entries; and (6) the ResourceValidator blast-radius rule — default injection
  must never live in the shared base validate() method; use
  validate_and_apply_defaults() on create paths instead.
managed_by: myco
user-invocable: true
...
registry and consuming validated output safely in tool implementations. Errors in
this layer cause silent data loss or live-controller corruption, so each step
carries a hard rule.
...
DEVICE_RADIO_UPDATE_SCHEMA = {
    ...
    "additionalProperties": False,  # Required on all *_UPDATE_SCHEMA — see §4
}
```
...
Adding them to update schemas enables accidental default injection — see §5 for
the full blast-radius explanation.
...
## Procedure 3: Hook the Validator into the Tool — the validated_data Contract
...
### The validated_data call pattern

`UniFiValidatorRegistry.validate()` is a **class method** that returns a 3-tuple:
...
**`registry.validate()` is a transformer, not a guard clause.** The third tuple
element (`validated_data`) is a validated copy of the input. Calling `validate()`
and then ignoring the third element — passing the original dict downstream instead
— silently discards type coercions and schema-applied transformations. The API call
succeeds but may receive wrong data.

This mistake has recurred across three PRs from three different contributors:

| PR | Tool | Mistake | Fix commit |
|---|---|---|---|
| #123 | `update_device_radio` | Return value discarded, original `updates` passed | `66fe214` |
| #126 | `update_snmp_settings` | `payload` used in API call instead of `validated_data` | `89a9998` |
| #146 | firewall policy tool | Same pattern | merge-blocked |

The pattern is systemic. It is not obvious because the call site *looks* correct — only
the missing capture on the downstream API call is wrong.
...
## Procedure 4: Layer-2 Hardening — additionalProperties: false
...
## Procedure 5: ResourceValidator Blast-Radius Rule
