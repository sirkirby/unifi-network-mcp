...
async def list_traffic_routes() -> Dict[str, Any]:
    ...
    try:
        ...
        routes_raw = [
            r.raw if hasattr(r, "raw") and isinstance(r.raw, dict) else {}
            for r in routes
        ]
        ...
        for idx, r_raw in enumerate(routes_raw):
            if isinstance(r_raw, dict) and r_raw.get("_id"):
                ...
                logger.warning(
                    f"Skipping invalid/incomplete route data at index {idx}: {r_raw}"
                )
...
async def get_traffic_route_details(route_id: str) -> Dict[str, Any]:
    ...
    if not parse_permission(config.permissions, "traffic_route", "read"):
        logger.warning(
            f"Permission denied for getting traffic route details ({route_id})."
        )
        return {
            "success": False,
            "error": "Permission denied to get traffic route details.",
        }
    try:
        if not route_id:
        ...
        route_obj = next(
            (
                ...
                for r in routes
                if hasattr(r, "raw")
                and isinstance(r.raw, dict)
                and r.raw.get("_id") == route_id
            ),
            None,
        )
        ...
        return {
            "success": True,
            "route_id": route_id,
            "details": json.loads(json.dumps(route_obj.raw, default=str)),
        }
    except Exception as e:
        logger.error(
            f"Error getting traffic route details for {route_id}: {e}", exc_info=True
        )
        return {"success": False, "error": str(e)}
...
async def toggle_traffic_route(route_id: str, confirm: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Attempting to toggle traffic route '{route_name}' ({route_id}) to {new_state}"
        )

        success = await firewall_manager.toggle_traffic_route(route_id)

        if success:
            toggled_route_obj = next(
                (
                    r
                    for r in await firewall_manager.get_traffic_routes()
                    if r.id == route_id
                ),
                None,
            )
            ...
            logger.info(
                f"Successfully toggled traffic route '{route_name}' ({route_id}) to {final_state}"
            )
            return {
                "success": True,
                "route_id": route_id,
            ...
            logger.error(
                f"Failed to toggle traffic route '{route_name}' ({route_id}). Manager returned false."
            )
            route_after_toggle_obj = next(
                (
                    r
                    for r in await firewall_manager.get_traffic_routes()
                    if r.id == route_id
            ...
            state_after = (
                route_after_toggle_obj.enabled if route_after_toggle_obj else "unknown"
            )
            return {
                "success": False,
                "route_id": route_id,
...
async def update_traffic_route(
    route_id: str, update_data: Dict[str, Any], confirm: bool = False
) -> Dict[str, Any]:
    ...
    is_valid, error_msg, validated_data = UniFiValidatorRegistry.validate(
        "traffic_route_update", update_data
    )
    if not is_valid:
        logger.warning(
            f"Invalid traffic route update data for ID {route_id}: {error_msg}"
        )
        return {"success": False, "error": f"Invalid update data: {error_msg}"}

    if not validated_data:
        logger.warning(
            f"Traffic route update data for ID {route_id} is empty after validation."
        )
        return {
            "success": False,
    ...
    try:
        ...
        logger.info(
            f"Attempting to update traffic route '{route_name}' ({route_id}) with fields: {updated_fields_list}"
        )
        ...
        success = await firewall_manager.update_traffic_route(
            route_id, validated_data
        )  # Pass only validated changes

        # 3. Fetch again to get the *actual* state after update attempt
        updated_route_obj = next(
            (
                r
                for r in await firewall_manager.get_traffic_routes()
                if hasattr(r, "raw")
        ...
        if success:
            logger.info(
                f"Successfully submitted update for traffic route '{route_name}' ({route_id})"
            )
            return {
                "success": True,
                "route_id": route_id,
            ...
            logger.error(
                f"Manager reported failure updating traffic route '{route_name}' ({route_id})."
            )
            return {
                "success": False,
                "route_id": route_id,
...
async def create_traffic_route(route_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if matching_target == "DOMAIN" and not validated_data.get("domains"):
        return {
            "success": False,
            "error": "Field 'domains' is required when matching_target is 'DOMAIN'.",
        }
    if matching_target == "IP" and not validated_data.get("ip_addresses"):
        return {
            "success": False,
            "error": "Field 'ip_addresses' is required when matching_target is 'IP'.",
        }
    if matching_target == "REGION" and not validated_data.get("regions"):
        return {
            "success": False,
            "error": "Field 'regions' is required when matching_target is 'REGION'.",
        }

    try:
        route_name = validated_data.get("name", "Unnamed Route")
        logger.info(
            f"Attempting to create traffic route '{route_name}' via interface '{validated_data.get('interface')}' with validated data."
        )
        ...
        if not validator:
            ...
            logger.error(
                "Could not retrieve validator for 'traffic_route' after successful validation."
            )
            return {
                "success": False,
                "error": "Internal error: Validator not found after validation.",
            }
        ...
        if result and result.get("success"):
            ...
            logger.info(
                f"Manager reported success creating traffic route '{route_name}'. ID: {new_id}"
            )
            return {
                "success": True,
                "route_id": new_id,
            ...
            error_msg = (
                result.get("error", "Unknown error during traffic route creation.")
                if isinstance(result, dict)
                else "Manager did not return a valid response."
            )
            logger.error(
                f"Failed to create traffic route '{route_name}'. Manager returned: {result}"
            )
...
async def create_simple_traffic_route(
    route: Dict[str, Any], confirm: bool = False
) -> Dict[str, Any]:
    ...
    net_obj = next(
        (
            n
            for n in nets
            if n.get("_id") == r["network"] or n.get("name") == r["network"]
        ),
        None,
    )
    if not net_obj:
    ...
    if r.get("client_macs"):
        targets = [
            {"type": "CLIENT", "client_mac": m.lower()} for m in r["client_macs"]
        ]
    else:
    ...
    if r["matching_target"] == "DOMAIN":
        ...
        payload["ip_addresses"] = [
            {"ip_or_subnet": ip} for ip in r.get("destinations", [])
        ]
    ...
    created = await firewall_manager.create_traffic_route(
        payload
