...
class HotspotManager:
    ...
    async def get_vouchers(
        ...
        Args:
            create_time: Optional Unix timestamp to filter vouchers by creation time.
        ...
        try:
            # Build request - GET for listing vouchers
    ...
    async def get_voucher_details(
        ...
        try:
            ...
            voucher = next(
                (v for v in all_vouchers if v.get("_id") == voucher_id), None
            )
            if not voucher:
    ...
    async def create_voucher(
        ...
        Args:
            ...
            note: Optional note for the voucher (e.g., for printing).
        ...
        try:
            ...
            logger.info(
                f"Create voucher command sent: {count} voucher(s), "
                f"{expire_minutes} min validity, quota={quota}"
            )

            # Invalidate cache after creating vouchers
            self._connection._invalidate_cache(
                f"{CACHE_PREFIX_VOUCHERS}_{self._connection.site}"
            )

            # Response contains create_time which can be used to fetch the new vouchers
            if isinstance(response, list) and len(response) > 0:
                create_time = response[0].get("create_time")
                if create_time:
                    # Fetch the newly created vouchers
                    new_vouchers = await self.get_vouchers(create_time=create_time)
                    return new_vouchers
                return response
    ...
    async def revoke_voucher(self, voucher_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Revoke command sent for voucher {voucher_id}")
