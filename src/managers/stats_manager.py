...
class StatsManager:
    ...
    def __init__(
        self, connection_manager: ConnectionManager, client_manager: ClientManager
    ):
    ...
    async def get_network_stats(self, duration_hours: int = 1) -> List[Dict[str, Any]]:
        ...
        cache_key = (
            f"{CACHE_PREFIX_STATS_NETWORK}_{duration_hours}_{self._connection.site}"
        )
        ...
        try:
            start_time = int(
                (datetime.now() - timedelta(hours=duration_hours)).timestamp() * 1000
            )
    ...
    async def get_top_clients(
        self, duration_hours: int = 24, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def get_alerts(
        ...
        cache_key = (
            f"{CACHE_PREFIX_STATS_ALERTS}_{include_archived}_{self._connection.site}"
        )
        cached_data = self._connection.get_cached(
            cache_key, timeout=60
        )  # 1 minute cache
        if cached_data is not None:
            return cached_data
